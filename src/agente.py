...
Usa o SDK novo e unificado do Google (google-genai). O pacote antigo
"google-generativeai" foi descontinuado pelo Google em 2025.
"""
...
def filtrar_por_pergunta(df: pd.DataFrame, pergunta: str) -> pd.DataFrame:
    """
...
def montar_contexto(pergunta: str, dados: dict) -> str:
    ...
    for chave in identificar_fontes_relevantes(pergunta):
        df = dados[chave]
        df_filtrado = filtrar_por_pergunta(df, pergunta)

        if not df_filtrado.empty and len(df_filtrado) < len(df):
            # O filtro por palavras da pergunta funcionou: usa esse recorte (até 300 linhas)
            amostra_df = df_filtrado.head(300)
        else:
            # Nenhum filtro específico bateu: usa uma amostra inicial do arquivo
            amostra_df = df.head(50)

        amostra = amostra_df.to_csv(index=False, sep=";")
...
def converter_historico(mensagens):
    ...
    for msg in mensagens:
        ...
        convertido.append({"role": papel, "parts": [{"text": msg["content"]}]})
    return convertido
...
def perguntar_ao_agente(pergunta: str, dados: dict, api_key: str, historico=None) -> str:
    ...
    chat = client.chats.create(
        model=MODELO_GEMINI,
        history=converter_historico(historico or []),
